...
#define get_U8(buffer, index, data)	{data = buffer[index++];}
#define put_U8(buffer, index, data)	{buffer[index++] = data;}

//Endian
//#define get_U16(buffer, index, data)	{(*(uint16_t*) &data) = (*(uint16_t *)(buffer + index)) ; index += 2;}
//#define get_U32(buffer, index, data)	{(*(uint32_t*) &data) = (*(uint32_t *)(buffer + index)) ; index += 4;}
#define get_U16(buffer, index, data){\
	((uint8_t*) &data)[0] = buffer[index++];\
	((uint8_t*) &data)[1] = buffer[index++];\
}
#define get_U32(buffer, index, data){\
	((uint8_t*) &data)[0] = buffer[index++];\
	((uint8_t*) &data)[1] = buffer[index++];\
...
//#define put_U16(buffer, index, data)	{(*(uint16_t *)(buffer + index)) = (*(uint16_t*) &data); index += 2;}
//#define put_U32(buffer, index, data)	{(*(uint32_t *)(buffer + index)) = (*(uint32_t*) &data); index += 4;}
#define put_U16(buffer, index, data){\
	buffer[index++] = ((uint8_t*) &data)[0];\
	buffer[index++] = ((uint8_t*) &data)[1];\
}
#define put_U32(buffer, index, data){\
	buffer[index++] = ((uint8_t*) &data)[0];\
	buffer[index++] = ((uint8_t*) &data)[1];\
